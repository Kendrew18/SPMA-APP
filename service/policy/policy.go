...
func Policy_R(Requests request.Request_Policy_R) (response.Response, error) {
	var res response.Response
	var arr_data []response.Response_Policy

	con := db.DB().Table("policy")

	err := con.Select("id_policy", "strata", "nama_product", "harga", "depo", "tanggal_update").Where("depo = ?", Requests.Depo).Order("co ASC").Scan(&arr_data).Error
...
// Policy Bonus
func Policy_Bonus(Requests []request.Request_Policy) (response.Response, error) {
	var res response.Response
	con := db.DB().Table("policy")
